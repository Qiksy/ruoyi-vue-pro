...
public class SqlInjectionUtil {	
	...
	private final static String SQL_INJECTION_FIELD_TIP = "字段不合法，存在SQL注入风险!---> {}";
	...
	public static void filterContent(String value, String customXssString) {
		...
		for (int i = 0; i < xssArr.length; i++) {
			if (value.indexOf(xssArr[i]) > -1) {
				...
				throw exception(SqlInjectionUtil.SQL_INJECTION_TIP + value);
			}
		}
	...
	public static String getSqlInjectTableName(String table) {
		...
		if (!isValidTableName) {
			log.error("表名不合法，存在SQL注入风险!--->{}",table);
			throw exception("表名不合法，存在SQL注入风险!--->{}",table);
		}
	...
	public static String getSqlInjectField(String field) {
		...
		if (!isValidField) {
			...
			log.error(errorMsg);
			throw exception(SqlInjectionUtil.SQL_INJECTION_FIELD_TIP ,field);
		}
